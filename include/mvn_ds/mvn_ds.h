...
typedef struct mvn_string_t {
  size_t
      length; /**< Current length of the string (excluding null terminator). */
...
typedef struct mvn_val_t {
  ...
  union {
    ...
    mvn_string_t
        *str;         /**< Pointer to owned string if type is MVN_VAL_STRING. */
    mvn_array_t *arr; /**< Pointer to owned array if type is MVN_VAL_ARRAY. */
    mvn_hmap_t
        *hmap; /**< Pointer to owned hash map if type is MVN_VAL_HASHMAP. */
...
mvn_val_t
mvn_val_string_take(mvn_string_t *str); // Takes ownership of an existing string
mvn_val_t mvn_val_array(void);          // Creates a new empty owned array
mvn_val_t
mvn_val_array_take(mvn_array_t *arr); // Takes ownership of an existing array
mvn_val_t mvn_val_hmap(void);         // Creates a new empty owned hash map
mvn_val_t
mvn_val_hmap_take(mvn_hmap_t *hmap); // Takes ownership of an existing map
 ...
 * failure.
 */
static bool mvn_string_ensure_capacity(mvn_string_t *string,
  ...
  size_t new_capacity = old_capacity < MVN_INITIAL_CAPACITY
  ...
  char *new_data =
      (char *)mvn_reallocate(string->data, old_capacity + 1, new_capacity + 1);
  if (!new_data)
...
mvn_string_t *mvn_string_new(const char *chars) {
  ...
  size_t initial_capacity =
      length < MVN_INITIAL_CAPACITY ? MVN_INITIAL_CAPACITY : length;
...
static bool mvn_array_ensure_capacity(mvn_array_t *array) {
  ...
  mvn_val_t *new_data =
      (mvn_val_t *)mvn_reallocate(array->data, old_capacity * sizeof(mvn_val_t),
                                  new_capacity * sizeof(mvn_val_t));
  if (!new_data)
    return false;
...
static mvn_hmap_entry_t *mvn_hmap_find_entry(mvn_hmap_entry_t **buckets,
  ...
  while (entry != NULL) {
    ...
    if (mvn_string_hash(entry->key) == hash &&
...
static void mvn_hmap_adjust_capacity(mvn_hmap_t *hmap, size_t new_capacity) {
  ...
  for (size_t i = 0; i < hmap->capacity; i++) {
    ...
    while (entry != NULL) {
      mvn_hmap_entry_t *next =
          entry->next; // Store next entry before modifying current
...
mvn_hmap_t *mvn_hmap_new_with_capacity(size_t capacity) {
  ...
  if (capacity > 0) {
    ...
    hmap->buckets =
        (mvn_hmap_entry_t **)calloc(capacity, sizeof(mvn_hmap_entry_t *));
    if (!hmap->buckets) {
      free(hmap);
      return NULL;
    }
...
bool mvn_hmap_set(mvn_hmap_t *hmap, mvn_string_t *key, mvn_val_t value) {
  ...
  if (hmap->count + 1 > hmap->capacity * MVN_MAP_LOAD_FACTOR) {
    ...
    if (new_capacity < hmap->capacity) {
      fprintf(
          stderr,
          "[MVN_DS] Hash map capacity overflow during resize calculation.\n");
  ...
  while (*bucket_ptr != NULL) {
    ...
    if (mvn_string_hash(entry->key) == hash &&
      ...
      mvn_string_free(
          key); // Free the *provided* key, as we keep the existing one
      return true;
    }
  ...
  mvn_hmap_entry_t *new_entry =
      (mvn_hmap_entry_t *)malloc(sizeof(mvn_hmap_entry_t));
  if (!new_entry) {
 ...
 * @return true if successful, false on allocation failure.
 */
bool mvn_hmap_set_cstr(mvn_hmap_t *hmap, const char *key_cstr,
                       mvn_val_t value) {
  if (!key_cstr) {
...
mvn_val_t *mvn_hmap_get(const mvn_hmap_t *hmap, const mvn_string_t *key) {
  ...
  mvn_hmap_entry_t *entry =
      mvn_hmap_find_entry(hmap->buckets, hmap->capacity, key, hash);
...
mvn_val_t *mvn_hmap_get_cstr(const mvn_hmap_t *hmap, const char *key_cstr) {
  ...
  mvn_string_t temp_key = {
      .length = strlen(key_cstr), .capacity = 0, .data = (char *)key_cstr};
  ...
  mvn_hmap_entry_t *entry = mvn_hmap_find_entry(
      ((mvn_hmap_t *)hmap)->buckets, hmap->capacity, &temp_key, hash);
  return entry ? &entry->value : NULL;
}
...
bool mvn_hmap_delete(mvn_hmap_t *hmap, const mvn_string_t *key) {
  ...
  size_t index = hash % hmap->capacity;
  mvn_hmap_entry_t **bucket_ptr =
      &hmap->buckets[index]; // Pointer to the link pointing to the current
                             // entry
  mvn_hmap_entry_t *entry = *bucket_ptr;
  ...
  while (entry != NULL) {
    ...
    if (mvn_string_hash(entry->key) == hash &&
      ...
      *bucket_ptr =
          entry->next; // Make the previous link point to the next entry
...
mvn_val_t mvn_val_null(void) { return (mvn_val_t){.type = MVN_VAL_NULL}; }
/** @brief Creates a boolean value. */
mvn_val_t mvn_val_bool(bool b) {
  return (mvn_val_t){.type = MVN_VAL_BOOL, .as.b = b};
}
/** @brief Creates a 32-bit integer value. */
mvn_val_t mvn_val_i32(int32_t i32) {
  return (mvn_val_t){.type = MVN_VAL_I32, .as.i32 = i32};
}
/** @brief Creates a 64-bit integer value. */
mvn_val_t mvn_val_i64(int64_t i64) {
  return (mvn_val_t){.type = MVN_VAL_I64, .as.i64 = i64};
}
/** @brief Creates a 32-bit float value. */
mvn_val_t mvn_val_f32(float f32) {
  return (mvn_val_t){.type = MVN_VAL_F32, .as.f32 = f32};
}
/** @brief Creates a 64-bit double value. */
mvn_val_t mvn_val_f64(double f64) {
  return (mvn_val_t){.type = MVN_VAL_F64, .as.f64 = f64};
}
...
mvn_val_t mvn_val_string(const char *chars) {
  ...
  return (mvn_val_t){.type = MVN_VAL_STRING, .as.str = s};
}
...
mvn_val_t mvn_val_string_take(mvn_string_t *str) {
  if (!str)
    return mvn_val_null();
  return (mvn_val_t){.type = MVN_VAL_STRING, .as.str = str};
}
...
mvn_val_t mvn_val_array(void) {
  ...
  if (!a)
    return mvn_val_null(); // Handle allocation failure
  return (mvn_val_t){.type = MVN_VAL_ARRAY, .as.arr = a};
}
...
mvn_val_t mvn_val_array_take(mvn_array_t *arr) {
  if (!arr)
    return mvn_val_null();
  return (mvn_val_t){.type = MVN_VAL_ARRAY, .as.arr = arr};
}
...
mvn_val_t mvn_val_hmap(void) {
  ...
  if (!m)
    return mvn_val_null(); // Handle allocation failure
  return (mvn_val_t){.type = MVN_VAL_HASHMAP, .as.hmap = m};
}
...
mvn_val_t mvn_val_hmap_take(mvn_hmap_t *hmap) {
  if (!hmap)
    return mvn_val_null();
  return (mvn_val_t){.type = MVN_VAL_HASHMAP, .as.hmap = hmap};
}
...
void mvn_val_free(mvn_val_t *value) {
  ...
  case MVN_VAL_STRING:
    mvn_string_free(value->as.str);
    break;
  case MVN_VAL_ARRAY:
    mvn_array_free(value->as.arr);
    break;
  case MVN_VAL_HASHMAP:
    mvn_hmap_free(value->as.hmap);
    break;
  ...
  default:
    fprintf(stderr,
            "[MVN_DS] Warning: mvn_val_free called on unknown type %d\n",
            value->type);
    break;
  }
...
void mvn_val_print(const mvn_val_t *value) {
  ...
  case MVN_VAL_ARRAY:
    ...
    printf("[");
    for (size_t i = 0; i < value->as.arr->count; i++) {
      mvn_val_print(&value->as.arr->data[i]);
      if (i < value->as.arr->count - 1) {
        printf(", ");
      }
    }
    printf("]");
  ...
  case MVN_VAL_HASHMAP:
    ...
    printf("{");
    bool first = true;
    for (size_t i = 0; i < value->as.hmap->capacity; i++) {
      mvn_hmap_entry_t *entry = value->as.hmap->buckets[i];
      while (entry) {
        if (!first) {
          printf(", ");
        }
